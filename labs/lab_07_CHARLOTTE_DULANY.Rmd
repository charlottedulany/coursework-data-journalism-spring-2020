...
```{r}
# In the not_in_population table, there is an inconsistent number of records between number of people and number of pills, because there are some people who did not obtain pills.
```
...
geo_pills_population_state_2009 %>%
  ...
  labs(fill='Population State',title="How pills compared per state", subtitle = "How dosage unit varied per region of the U.S.", caption = "Source: U.S. Census ACS")
```
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Pill shipments spike in 2007 before drastically falling until 2012", subtitle = "Dosage units saw their height in 2007 before trickling off for the next five years", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
...
pills_per_person_in_each_county_2009 <- pills_population %>%
  filter(year==2009) %>%
  group_by(statefips, buyer_state) %>%
  summarise(total_pills=sum(dosage_unit, na.rm = TRUE),
            total_population=sum(population, na.rm=TRUE)) %>%
...
geo_pills_population_county_Maryland_per_year %>%
  ggplot(aes(fill = pills_per_person)) +
  facet_wrap(~year, nrow=2) +
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person in Maryland ',title="Pills per person in Maryland", subtitle = "How dosage unit varied in Maryland counties", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
