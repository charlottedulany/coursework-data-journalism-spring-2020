...
add.column(mingo_working, mingo)
  mutate(buyer_address_complete = paste0(buyer_address1," ", buyer_city, ", ", buyer_state, " ", buyer_zip)) 
...
mingo_working <- mingo %>%
  mutate(state_distribution = if_else(in_state_distribution = "West Virginia", out_of_state_distribution = 
...
```{r}
mingo %>%
  group_by(manufacturer_name, distributor_name, pharmacy_name) %>%
...
```{r}
mingo %>%
  group_by(distributor name = Cardinal Health) %>%
  arrange(desc(buyer_zip))
...
I'm not sure if this is a possibility, but if you could organize by the word "Cardinal" it would most likely sort them in a group together, because no other distributor shares this name.
