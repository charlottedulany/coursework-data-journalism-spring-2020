...
```{r}
Address <- read_csv("data/buyer_addresses.csv")
Totals <- read_csv("data/buyer_totals.csv")
Annual <- read_csv("data/buyer_annual_by_year.csv")
...
# There are different numbers of records for the buyers because there is overlapping in some of the categories, including buyer_address and buyer_totals. Buyer_annual_by_year is higher than buyer_totals because it is analyzing the total dosage_unit of each buyer for each year, whereas buyer_totals is analyzing the sum of dosage_unit from one buyer for all years. Buyer_addresses is higher than buyer_totals because it includes addresses of hospitals, doctors offices, pharmacies and others that may or may not distribute opioid pills. 
...
# The VA CONSOLIDATED MAIL OUTPATIENT PHARMACY in North Charleston, SC at 3725 Rivers Avenue had the most pills sent to it, for a total of 512,414,034 opioid pills. According to Post and Courier, this pharmacy sought to market their products to veterans. It is a mail-order warehouse that carried out an "orchestrated effort" to serve painkillers to those who have served.
...
Annual %>%
  ...
  inner_join(Address, by="buyer_dea_no")

# RAKESH K MATHUR, MD is the practitioner in Maryland who had the post pills in 2010, for a total of 337,450 pills. Compared to the next highest doctor in the state that year, SRIRAM H BALASUBRAMANIAN, MATHUR had 124,650 more pills than him that year. Using MATHUR's buyer_dea_no I am able to make sure that the doctor I find on the stateboard search tool is the same doctor described in this data. MATHUR's license is currently on probation and he had to pay a fine of $25,000 with a minimum of five years with terms and conditions to The Board in 2014.
...
# By using a right_join of buyer_addresses and buyer_totals I am sorting through the data to see if buyer_name values change <- that way it will show me if any buyers in one table are not included in the other.
...
# California has the highest population, so next time I will check how the totals for California compares to states with smaller populations.
```
