...
options(scipen=999)
  ...
  geom_bar(stat="identity", aes(year, dosage_unit), fill="purple") +
     labs(x="Year", y="Total pills", title="In Maryland, opioid prescriptions climb before trickling off", subtitle="Opioid prescriptions throughout Maryland from 2006 to 2014") +
...
```{r}
# The year-by-year trend of opioid shipments in Maryland began comparatively low in 2006 before spiking and then falling gradually for the following years of the study. The year-by-year trend of such in Baltimore City began with at a high rate before dropping, somewhat rising and then more or less reaching a plateau.
...
# Create a dataframe of state population in 2012
state_population_2012 <- state_population(key = key) %>%
  filter(year=="2012")


united_states_2012 <- arcos_county_pills_per_year %>%
  clean_names() %>%
  filter(year=="2012") %>%
  group_by(buyer_state) %>%
  summarize(total_pills=sum(dosage_unit))

us_pills_population_2012 <- state_population_2012 %>%
...
ggplot(united_states_2012) +
  ...
  labs(x="2012 Population", y="Total pills in 2012", title="High number of opioid prescriptions sweep the nation", caption = "Source: DEA ARCOS database, via Washington Post", fill="buyer_state") +
    scale_y_continuous(labels = comma) +
...
```{r}
pills_per_person_2012 <- arcos_county_pills_per_year %>%
  inner_join(united_states_2012, by=("buyer_state")) %>%
